...
export class ReceiptComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        console.log("Quyen debug full url: ", this.window.location.href)
        var parser = this.router.parseUrl(this.router.url)
        this.Status = parseInt(parser.queryParams['status'].toString());
        console.log("Quyen debug status: ", this.Status)
        this.loadPageData(1)
    }
    ...
    private loadPageData(page: number) {
        ...
        if (localToken != undefined) {
            if (localToken.length != 0) {
                var host = "http://" + this.window.location.hostname;
                this.service.GetReceiptList(page, this.Status, localToken, "", host).then((res) => {
